...
//                m_IdentecPour = new IdentecPour { Driver = Config.Name };
        ...
        override public void Start( )
        {
            try
            {
                ...
                if( null == m_RetryWatcher || m_RetryWatcher.Status != TaskStatus.Running)
                {
                    m_RetryWatcher = Task.Factory.StartNew(RestartThread);
                }
        ...
        override public void Stop()
        {
            try
            {
                ...
                m_MessageProcessor.Join(1000);
                m_TagReader.Join(1000);
            }
            catch (Exception exp)
            {
        ...
        protected void Restart( )
        {
            ...
            {
                Log.WriteException("IdentecDev::Restart", exp);
            }
        }
            ...
            try
            {
                if( Config.Options.Count != 0 )
                {
                    ...
                    if( null != m_ILR350Reader )
                    {
                        ...
                        m_ILR350Reader.ResetToFactoryDefault();
                        m_ILR350Reader.SetRFBeaconBaudrate( Config.GetBaudRate( ) );
                        m_ILR350Reader.SetFrequency(Config.GetFrequency());
                        m_ILR350Reader.TXPower = 10;

                        Log.Debug(string.Format("Transmit power: {0} - Filter Level {1}", m_ILR350Reader.TXPower, m_ILR350Reader.GetTagSignalFilterLevel()));

                        m_AllowPours = Config.GetPublishPours( );
                        m_AllowHeartbeat = Config.GetPublishHeartbeats( );

//                        m_ILR350Reader.SetTagSignalFilterLevel()
        ...
        private void SendAlertMessage( AlertTypes _type, ILR350Tag _tag, Dictionary<string, object> _parameters )
        {
            try
            {
                ...
                if (null != beaconMessage)
                {
                    ...
                    message.Parameters["EventTime"] = message.ReadTime;
                    message.Parameters["TagID"] = _tag.SerialNumber;
                    message.Parameters["DeviceID"] = message.DeviceID;

                    SendAlert(message, _type, string.Format("{0} {1} {2}", message.DeviceID, message.ReadTime, _tag.SerialNumber), true);
                }
            }
            catch( Exception exp )
            {
...
#warning not sending alerts for all msg, only pours
        ...
        private void SendMessage( ILR350Tag _tag )
        {
            try
            {
                if( _tag != null )
                {
                    if ( _tag.BatteryStatus == BatteryStatus.Poor )
                    {
                        SendAlertMessage( AlertTypes.LowBattery, _tag, null );
                    }
