...
namespace Jaxis.Inventory.Data
{
    public class PourBLManager : BLManager<IPour, IBLPour>, IPourBLManager
    {
        ...
        public IEnumerable<IBLPour> GetByTicketItem(IBLPOSTicketItem _ticketItem)
        {
            return (from p in DataManagerFactory.Get().Manage<IPour>().GetAll()
                    where p.POSTicketItemID == _ticketItem.ObjectID
                    select p).ToList().Cast<IBLPour>();
